...
**The Whole-Flyer Rule.** A flyer is never cropped to fit a box. Club posts arrive in every Instagram shape (4:5 and 3:4 portrait, 1:1, 9:16 reel covers, landscape), so each surface gives the flyer a fixed slot for rhythm and shows the flyer whole, at its own shape, inside it. The frame (radius, hairline, shadow) is drawn on the flyer itself, never on an empty crop box. `FlyerPoster` and `.flyer-fit` implement it: the slot sets `--flyer-max-w` / `--flyer-max-h`, and the image sizes the poster itself in CSS, so a flyer in the server's HTML paints at its own shape on first paint, with no script. Until the image loads, the poster holds a 4:5 placeholder, so space is reserved and nothing shifts. Shapes narrower than 4:5 (reel covers) keep the placeholder's width until script reads their shape; `--flyer-anchor` pins the image to the slot's side so it never moves when the frame narrows. Where the flyer sits above text (the phone detail hero), a shape that arrives after first paint centers in the reserved space instead of resizing it. One exception: a wall tile fills edge to edge when the flyer is within ~8% of the tile's shape, because a trim that small cannot be seen; every other shape is pinned whole on the tile. (The earlier story-era thumbnail, a tall crop stretched to the row's height, cut the sides off every post.)
...
- **Do** shimmer anything that is loading: skeleton bars and a flyer's slot until the flyer arrives carry `.skeleton`, a band of light sweeping left to right at one linear speed (the one motion that is not eased out, because it loops). Flyers that mount in the browser fade in over their slot (300ms, ease-out); flyers in the server's HTML paint as soon as they arrive, never held back for JS. A flyer's slot waits 400ms before its first sweep, so a cached flyer never flashes one. Skeletons are for in-app waits only: a direct load of /events or an event (a refresh, a shared link) waits for its data and paints whole, because a route skeleton streams ahead of data the page is about to have and flashes on every refresh. In-app links onto the feed work as Luma's do (`EventsFeedLink`): the feed is prefetched while the reader is elsewhere, so a click usually lands at once, and when it is not ready the click shows the feed's own skeleton straight away (after a 100ms beat, so a near-instant navigation never flashes it) until the page arrives. Never leave the old page up with no sign a click registered.
