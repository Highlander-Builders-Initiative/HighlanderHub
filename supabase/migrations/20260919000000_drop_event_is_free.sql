-- events.is_free is no longer extracted and the app no longer reads or edits
-- it. Stop the publish RPC from writing it, then drop it.
-- plpgsql bodies aren't dependency-tracked, so the function must be replaced
-- before the column goes or every publish would fail at runtime.
