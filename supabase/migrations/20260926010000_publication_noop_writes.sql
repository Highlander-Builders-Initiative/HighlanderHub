-- Suppress unchanged publication writes inside the ownership transaction.
-- scraped_at changes only when the business projection changes.
  ...
  for item in select value from jsonb_array_elements(updates) loop
    ...
    if item->'assessment'->>'status' = 'error' then
      ...
      on conflict(source_key) do update set
        ...
        updated_at = now()
      where source_assessments.assessment is distinct from excluded.assessment
        or not source_assessments.event_ids @> excluded.event_ids
        or not source_assessments.known_event_ids @> excluded.known_event_ids;
      continue;
    end if;
    ...
    on conflict(source_key) do update set
      ...
      updated_at = now()
    where source_assessments.assessment is distinct from excluded.assessment
      or source_assessments.last_complete_assessment is distinct from excluded.last_complete_assessment
      or not (source_assessments.event_ids @> excluded.event_ids
              and source_assessments.event_ids <@ excluded.event_ids)
      or not (source_assessments.known_event_ids @> excluded.known_event_ids
              and source_assessments.known_event_ids <@ excluded.known_event_ids);
    ...
    for candidate in select value from jsonb_array_elements(item->'rows') loop
      ...
      where row(
          events.title, events.description, events.starts_at, events.ends_at, events.location, events.host, events.host_handle, events.all_day, events.category, events.content_kind, events.tags, events.source, events.source_url, events.image_url, events.has_free_food, events.rsvp_required, events.rsvp_url
        ) is distinct from row(
          excluded.title, excluded.description, excluded.starts_at, excluded.ends_at, excluded.location, excluded.host, excluded.host_handle, excluded.all_day, excluded.category, excluded.content_kind, excluded.tags, excluded.source, excluded.source_url, excluded.image_url, excluded.has_free_food, excluded.rsvp_required, excluded.rsvp_url
        )
        and not events.is_locked
