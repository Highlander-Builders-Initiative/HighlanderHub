-- Limit publication and duplicate retirement to the supported Instagram importer.
-- Historical rows and assessment records remain untouched.
...
begin
  ...
  for item in select value from jsonb_array_elements(removals) loop
    ...
    if not starts_with(existing.id, 'ig_')
       or existing.source is distinct from 'instagram' then
