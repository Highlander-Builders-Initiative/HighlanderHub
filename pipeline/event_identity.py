...
def event_key(row: dict[str, Any]) -> str | None:
    ...
    # Both legacy <owner>_<minute> and current <owner>_p<media_id>[-<session>]
    # IDs keep the owner intact when their final underscore-delimited component
    # is removed.
    row_id = str(row.get("id") or "")
...
def _row_score(row: dict[str, Any]) -> tuple[int, int, int, int, int, int, int, str]:
